...
### Perf / Fraîcheur — Les sauvegardes de l'espace éditeur revalident enfin les pages où elles se voient
- **Constat, à l'occasion du renommage** : le nouveau nom était immédiat sur la fiche et `/solutions` (couverts par `revalidateSolution()`), mais mettait jusqu'à **1 h** à apparaître sur la **page catégorie** et la **page éditeur** (ISR 3600 s). Pire, `updateEditeurByUser()` — l'onglet « Page éditeur » — ne revalidait **rien du tout** : un nom commercial, un logo ou un mot de l'éditeur modifié là restait invisible jusqu'à 1 h sur `/editeur/[slug]` **et** sur l'annuaire `/editeurs`.
- **Fix** : helper `revalidatePagesEditeur()` ([admin-users.ts](src/lib/actions/admin-users.ts)) appelé sur les **deux** chemins de sauvegarde de l'espace éditeur → `/solutions/{cat}` + `/editeur/{slug}` côté solution, `/editeur/{slug}` + `/editeurs` côté page éditeur.
- ⚠️ **Pourquoi ne PAS l'avoir mis dans `revalidateSolution()`** : cette fonction est sur le **chemin chaud des évaluations** (appelée à chaque notation), et c'est précisément là qu'un périmètre trop large avait coûté cher (cf. 2026-08-08). Le chemin « l'éditeur enregistre » est **rare** et sans rapport — d'où un helper séparé plutôt qu'un élargissement du commun.
- **Coût chiffré avant de le faire** : `revalidatePath` ne recalcule rien, il pose un marqueur ; le re-rendu est payé à la visite suivante, et **ces pages se re-rendaient de toute façon dans l'heure** — on anticipe, on n'ajoute pas de rendu récurrent. Volume réel mesuré sur `editeurs_edit_log` : **94 sauvegardes en 3 mois, 59 sur 30 jours ≈ 2/jour**. Majorant pessimiste ≈ 120 rendus/mois × ~0,5 s ≈ **1 min/mois** contre un plafond de 240 min et une consommation actuelle de ~140 min/mois → **sous le bruit de mesure**.
