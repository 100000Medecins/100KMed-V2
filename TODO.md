...
- **Phase 5 livrée (2026-05-25)** : `<Card>` créé ([src/components/ui/Card.tsx](src/components/ui/Card.tsx)). Props : `padding` (none/sm/md/lg/xl), `hoverable` (cards cliquables), `overflow` (hidden/visible), forward de tous les attrs HTML standards. 4 fichiers migrés en démo : `mes-favoris/page.tsx` (2 cards : empty state + items), `mes-evaluations/page.tsx` (empty state), `mes-questionnaires-these/page.tsx` (empty state), `admin/editeurs-log/page.tsx` (2 cards : empty + table wrapper avec `padding="none"`). **Reste à migrer au fil de l'eau** : ~85 autres usages de `bg-white rounded-card shadow-card` dans le projet.
- **Toutes les phases sont livrées** ✅ — ce qui reste = migrations au fil de l'eau dans les fichiers qu'on touche pour d'autres raisons. Pas de chantier dédié.
