...
import {
  ...
  CalendarDays, Plus, Pencil, Archive, X, Send,
} from 'lucide-react'
...
import PublishEmailModal, { type PublishEmailConfig } from '@/components/admin/PublishEmailModal'
...
function QuestionnairesSection({ questionnaires: initial }: { questionnaires: QuestionnaireThese[] }) {
  ...
  const [publishModal, setPublishModal] = useState<PublishEmailConfig | null>(null)
  ...
  const openPublishModal = (q: QuestionnaireThese) => {
    setPublishModal({
      type: 'questionnaire',
      id: q.id,
      titre: q.titre,
      description: q.description,
      lienInitial: q.lien,
      specialitesInitiales: q.specialites_cibles ?? [],
    })
  }

  const handlePublished = () => {
    setQuestionnaires((prev) => prev.map((q) => q.id === publishModal?.id ? { ...q, statut: 'publie' } : q))
  }
  ...
  const handleRefuse = (id: string) => {
    ...
    startTransition(async () => {
      ...
      const result = await setStatutQuestionnaire(id, 'refuse')
      if (!result.error) setQuestionnaires((prev) => prev.map((q) => q.id === id ? { ...q, statut: 'refuse' } : q))
      setPendingId(null)
    })
  }
                        ...
                        <div className="flex items-center gap-1.5 ml-auto">
                          {isLoading ? (
                            ...
                            <>
                              ...
                              {q.statut !== 'publie' && (
                                <button onClick={() => openPublishModal(q)} disabled={isPending}
                              ...
                              {q.statut !== 'refuse' && (
                                <button onClick={() => handleRefuse(q.id)} disabled={isPending}
...
type EtudeTab = 'en_attente' | 'actives' | 'archives' | 'refuse'

const ETUDE_TABS: { key: EtudeTab; label: string }[] = [
  { key: 'en_attente', label: 'En attente' },
...
function filterEtude(tab: EtudeTab, e: EtudeClinique, t: string): boolean {
  if (tab === 'en_attente') return e.statut === 'en_attente'
  if (tab === 'refuse') return e.statut === 'refuse'
  const expired = e.date_fin && e.date_fin < t
  if (tab === 'actives') return e.statut === 'publie' && !expired
  if (tab === 'archives') return e.statut === 'publie' && !!expired
  return false
}

function EtudesSection({ etudes: initial }: { etudes: EtudeClinique[] }) {
  const [etudes, setEtudes] = useState(initial)
  const [activeTab, setActiveTab] = useState<EtudeTab>('en_attente')
  ...
  const counts: Record<EtudeTab, number> = {
    en_attente: etudes.filter((e) => filterEtude('en_attente', e, t)).length,
    actives: etudes.filter((e) => filterEtude('actives', e, t)).length,
    archives: etudes.filter((e) => filterEtude('archives', e, t)).length,
    refuse: etudes.filter((e) => filterEtude('refuse', e, t)).length,
  }

  const filtered = etudes.filter((e) => filterEtude(activeTab, e, t))
  ...
  const handleRefuse = (id: string) => {
    ...
    startTransition(async () => {
      ...
      const result = await setStatutEtude(id, 'refuse')
      if (!result.error) setEtudes((prev) => prev.map((e) => e.id === id ? { ...e, statut: 'refuse' } : e))
      setPendingId(null)
    })
  }

  const openPublishModal = (e: EtudeClinique) => {
    setPublishModal({
      type: 'etude',
      id: e.id,
      titre: e.titre,
      ...
      lienInitial: e.lien ?? '',
      specialitesInitiales: e.specialites_cibles ?? [],
    })
  }

  const handlePublished = () => {
    setEtudes((prev) => prev.map((e) => e.id === publishModal?.id ? { ...e, statut: 'publie' } : e))
  }
  ...
  return (
    <>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-1 bg-white rounded-xl shadow-card p-1 w-fit flex-wrap">
            {ETUDE_TABS.map(({ key, label }) => (
              <button key={key} onClick={() => setActiveTab(key)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  activeTab === key ? 'bg-navy text-white' : 'text-gray-500 hover:text-navy hover:bg-surface-light'
                }`}>
                {key === 'archives' && <Archive className="w-3 h-3" />}
                {key === 'en_attente' && <Clock className="w-3 h-3" />}
                {key === 'actives' && <FlaskConical className="w-3 h-3" />}
                {label}
                {counts[key] > 0 && (
                  <span className={`text-xs rounded-full px-1.5 font-semibold ${activeTab === key ? 'bg-white/20' : key === 'en_attente' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100'}`}>
                    {counts[key]}
                  </span>
                )}
              </button>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-card shadow-card p-10 text-center">
            ...
            <p className="text-sm text-gray-500">
              {activeTab === 'en_attente' ? 'Aucune étude en attente de validation.' :
               activeTab === 'actives' ? 'Aucune étude clinique active.' :
               activeTab === 'refuse' ? 'Aucune étude refusée.' :
               'Aucune étude clinique archivée.'}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={`shrink-0 inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${statutCfg.className}`}>
                        ...
                        <div className="flex items-center gap-1.5 ml-auto">
                          {isLoading ? (
                            ...
                            <>
                              ...
                              {e.statut !== 'publie' && (
                                <button onClick={() => openPublishModal(e)} disabled={isPending}
                                  className="flex items-center gap-1 px-2.5 py-1.5 bg-green-50 text-green-700 hover:bg-green-100 text-xs font-medium rounded-lg transition-colors disabled:opacity-50">
                                  <Send className="w-3.5 h-3.5" />
                                  Publier
                                </button>
                              ...
                              {e.statut !== 'refuse' && (
                                <button onClick={() => handleRefuse(e.id)} disabled={isPending}
                                  className="flex items-center gap-1 px-2.5 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 text-xs font-medium rounded-lg transition-colors disabled:opacity-50">
                                  <XCircle className="w-3.5 h-3.5" />
                                  Refuser
                                </button>
