...
// Bloquer le sitemap hors prod : evite que Google decouvre les URLs via le sitemap
// d'un environnement preview/dev (en complement du robots.txt qui Disallow: /).
...
/**
 * Construit la liste des URLs du sitemap (pages statiques + catégories + solutions
 * + articles + éditeurs actifs).
 *
 * Partagée par la route Next `app/sitemap.ts` (→ /sitemap.xml) ET par la route XML
 * alternative `app/sitemap-main.xml` (URL NEUVE servant le même contenu, pour repartir
 * sur une entité GSC sans l'historique « Impossible de récupérer » collant de
 * /sitemap.xml — cf docs/2026-07-19-audit-seo-indexation-gsc.md).
 *
 * Ne 5xx JAMAIS : sur hoquet BDD (cold start, réseau…), replie sur les pages statiques —
 * Googlebot journaliserait sinon un « Impossible de récupérer le sitemap ».
 */
export async function getSitemapEntries(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    // Hoquet Supabase (cold start, réseau…) : ne JAMAIS 5xx. On sert au moins les pages statiques.
