...
// TTL du lien de confirmation d'inscription : 7 jours
...
function sign(userId: string, iat: number): string {
  return createHmac('sha256', secret())
    .update(`confirm:${userId}:${iat}`)
    .digest('hex')
}

/** Génère un token HMAC idempotent (rejouable) pour confirmer une inscription. */
export function generateConfirmToken(userId: string): { iat: number; token: string } {
...
export function verifyConfirmToken(userId: string, iat: number, token: string): VerifyResult {
