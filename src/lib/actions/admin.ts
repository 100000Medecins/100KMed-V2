...
// Détecte si un meta.title suit un pattern auto-généré (= safe à recalculer).
// Sinon → c'est un override manuel, on respecte.
function isAutoGeneratedTitle(title: string | null): boolean {
  if (!title) return false
  // Patterns actuels (accroche adaptative — cf. buildSolutionSeoTitle)
  if (title.startsWith('Les avis de vos confrères sur ')) return true
  if (title.startsWith('Les avis sur ')) return true
  // Pattern legacy ( — Avis médecins | catégorie )
  if (title.includes(' — Avis médecins | ')) return true
  return false
}

// Parse un champ caché JSON de contacts (envoyé par SolutionForm) en liste propre.
function parseContactsField(raw: FormDataEntryValue | null): ContactLigne[] {
...
function extractSolutionFromFormData(formData: FormData) {
  ...
  const nomSeo = (formData.get('nom_seo') as string)?.trim() || null
  ...
  return {
    ...
    contacts_commerciaux: parseContactsField(formData.get('contacts_commerciaux_json')),
    contacts_support: parseContactsField(formData.get('contacts_support_json')),
    ...
    meta: (() => {
      const submittedTitle = (formData.get('meta_title') as string) || null
      const d = (formData.get('meta_description') as string) || null
      const c = (formData.get('meta_canonical') as string) || null

      // Auto-recompute du title si le submitted suit un pattern auto-généré
      // (pas un override manuel). Ça permet de fixer nom_seo et de voir le
      // <title> se mettre à jour automatiquement au save, sans passer par
      // "Générer le SEO" ni le script de migration.
      let t = submittedTitle
      if (isAutoGeneratedTitle(submittedTitle)) {
        const { title: recomputed, overflow } = buildSolutionSeoTitle({ nom, nom_seo: nomSeo })
        if (!overflow) t = recomputed
        // Si overflow: on garde le submittedTitle inchangé. L'UI a déjà
        // alerté l'utilisateur via l'aperçu live.
      }
...
export async function setDisplayContactsCommerciaux(value: boolean) {
  ...
  revalidatePath('/admin/parametres')
  revalidatePath('/solutions', 'layout')
  // Réglage global : le bloc commercial apparaît/disparaît sur TOUTES les fiches.
  // Le layout seul ne revalide pas les fiches (route dynamique ISR) → on invalide
  // le cache de toutes les instances de la route fiche.
