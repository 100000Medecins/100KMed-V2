// Envoie un email de test en utilisant un vrai template Supabase + withEmailLogo
// Permet de vérifier que le logo s'affiche correctement dans un vrai client email
...
// === Logo couleur base64 (identique à src/lib/email/logo.ts) ===
...
// Même logique que withEmailLogo dans logo.ts
function withEmailLogo(html) {
  const LOGO_ROW = `<tr><td style="padding:20px 0 2px;text-align:left;"><a href="https://www.100000medecins.org" style="display:block;text-decoration:none;line-height:0;margin-left:-52px;"><img src="${LOGO_DATA_URI}" alt="100 000 Médecins" width="425" height="100" style="display:block;height:100px;width:auto;" /></a></td></tr>`
  const anchor = 'style="max-width:580px;width:100%;">'
  const idx = html.indexOf(anchor)
  if (idx !== -1) {
    const pos = idx + anchor.length
    return html.slice(0, pos) + LOGO_ROW + html.slice(pos)
  }
  const bodyMatch = html.match(/<body[^>]*>/i)
  if (bodyMatch) return html.replace(bodyMatch[0], bodyMatch[0] + LOGO_ROW)
...
let html = template.contenu_html.replace(/\{\{(\w+)\}\}/g, (_, key) => sampleValues[key] ?? `{{${key}}}`)
html = withEmailLogo(html)

const sujet = `[TEST LOGO — ${templateId}] ${template.sujet}`
