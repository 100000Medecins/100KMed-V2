// Injecte le logo couleur directement dans tous les templates Supabase.
// - Header : logo comme premier <tr> dans le conteneur 580px, taille adaptée au conteneur
// - Footer : remplace les dots (●●●) par le logo image
// Après ce script, withEmailLogo() dans logo.ts n'est plus nécessaire pour ces templates.
// Usage : node scripts/bake-logo-in-templates.mjs

import { createClient } from '@supabase/supabase-js'
import { readFileSync } from 'fs'

const raw = readFileSync(new URL('../.env.local', import.meta.url), 'utf-8')
const env = Object.fromEntries(
  raw.split('\n').filter(l => l.includes('=')).map(l => {
    const i = l.indexOf('=')
    return [l.slice(0, i).trim(), l.slice(i + 1).trim()]
  })
)

const supabase = createClient(env['NEXT_PUBLIC_SUPABASE_URL'], env['SUPABASE_SERVICE_ROLE_KEY'])

const S = 'https://www.100000medecins.org'

// Logo couleur base64 (logo-secondaire-couleur-500.png)
const LOGO_DATA_URI = 'data:image/png;base64,'

// Logo header : 280px large, aligné à gauche, padding top pour espacer du bord
const LOGO_HEADER = `<tr><td style="padding:20px 0 10px;text-align:left;"><a href="${S}" style="display:block;text-decoration:none;line-height:0;"><img src="${LOGO_DATA_URI}" alt="100 000 Médecins" width="280" height="66" style="display:block;width:280px;height:auto;" /></a></td></tr>`

// Footer propre avec logo (remplace les dots ●●●)
const FOOTER_ROW_LOGO = `<img src="${LOGO_DATA_URI}" alt="100 000 Médecins" width="200" height="47" style="display:block;width:200px;height:auto;margin:0 auto 8px;" />`

// Regex pour repérer le bloc "dots footer" — les 3 spans colorés + texte
// On remplace tout le <p> contenant les dots par le logo
const FOOTER_DOTS_RE = /<p[^>]*>\s*<span[^>]*background:#4A90D9[^>]*><\/span>\s*<span[^>]*background:#E8734A[^>]*><\/span>\s*<span[^>]*background:#F5A623[^>]*><\/span>[^<]*<span[^>]*100\s*000\s*Médecins[^<]*<\/span>\s*<\/p>/s

// Regex pour retirer un éventuel logo déjà injecté par withEmailLogo (src="data:image/png;base64,...")
const INJECTED_LOGO_RE = /<tr><td[^>]*><a href="https:\/\/www\.100000medecins\.org"[^>]*><img src="data:image\/png;base64,[^"]*"[^>]*\/><\/a><\/td><\/tr>/g

function processTemplate(html) {
  // 1. Supprimer tout logo précédemment injecté par withEmailLogo
  html = html.replace(INJECTED_LOGO_RE, '')

  // 2. Injecter le nouveau logo en header (après le conteneur 580px)
  const anchor = 'style="max-width:580px;width:100%;">'
  const idx = html.indexOf(anchor)
  if (idx !== -1) {
    const pos = idx + anchor.length
    html = html.slice(0, pos) + '\n' + LOGO_HEADER + html.slice(pos)
  }

  // 3. Remplacer les dots du footer par le logo
  if (FOOTER_DOTS_RE.test(html)) {
    html = html.replace(FOOTER_DOTS_RE, `<p style="margin:0 0 8px;text-align:center;">${FOOTER_ROW_LOGO}</p>`)
  }

  return html
}

// Récupérer tous les templates
const { data: templates, error } = await supabase
  .from('email_templates')
  .select('id, contenu_html')

if (error) {
  console.error('❌ Erreur Supabase:', error.message)
  process.exit(1)
}

console.log(`📧 ${templates.length} templates trouvés\n`)

let updated = 0
let skipped = 0

for (const t of templates) {
  const newHtml = processTemplate(t.contenu_html)

  if (newHtml === t.contenu_html) {
    console.log(`  ⏭  ${t.id} — inchangé (ancre 580px introuvable ou déjà à jour)`)
    skipped++
    continue
  }

  const { error: err } = await supabase
    .from('email_templates')
    .update({ contenu_html: newHtml })
    .eq('id', t.id)

  if (err) {
    console.error(`  ❌ ${t.id} — erreur : ${err.message}`)
  } else {
    console.log(`  ✅ ${t.id} — mis à jour`)
    updated++
  }
}

console.log(`\n✅ ${updated} mis à jour, ${skipped} inchangés`)
