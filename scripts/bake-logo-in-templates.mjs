// Restauration complète des 11 templates email + logo via URL absolue
// Remplace les précédentes tentatives base64 (bloquées par Gmail/Outlook)
...
const LOGO_URL = `${S}/logos/logo-secondaire-couleur-500.png`

// Logo header via URL absolue — compatible Gmail, Outlook, Apple Mail
const HEADER = `
  <tr>
    <td style="padding:20px 0 16px;">
      <a href="${S}" style="display:block;text-decoration:none;line-height:0;">
        <img src="${LOGO_URL}" alt="100 000 Médecins" width="220" style="display:block;width:220px;height:auto;" />
      </a>
    </td>
  </tr>`

// Footer avec les dots originaux
const FOOTER = `
  <tr>
...
// ── Lancement (template spécial multi-sections) ───────────────────────────────
...
console.log(`\n📧 Restauration de ${templates.length} templates email…\n`)

for (const tpl of templates) {
  const hasBase64 = tpl.contenu_html.includes('data:image/png;base64')
  const has580    = tpl.contenu_html.includes('max-width:580px')
  const hasLogo   = tpl.contenu_html.includes(LOGO_URL)
  const hasDots   = tpl.contenu_html.includes('background:#4A90D9;margin-right:2px')

  if (hasBase64) {
    console.error(`  ⚠️  ${tpl.id} — base64 détecté dans le HTML généré ! Abandon.`)
    continue
  }
  ...
  if (error) {
    ...
    console.log(`  ✅ ${tpl.id} — 580px:${has580} logoURL:${hasLogo} dots:${hasDots}`)
  }
}
